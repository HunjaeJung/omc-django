...
def store_register_save(request):
    if request.method == "POST":
        ...
        store.manager_name = req['manager_name']
        store.manager_phone = req['manager_phone']
        store.address = req['address']
        store.description = req['description']
        # 주소 관련 데이터 나중 참고 피료
        
        store.longitude = float('%.7f' % float(req['lng']))
        store.latitude = float('%.7f' % float(req['lat']))

        wl = ''
        week_list = request.POST.getlist("week")
        for w in week_list:
            if week_list.index(w) == 0:
                wl += w
            else:
                wl += ',' + w
        store.dayoff = wl
        store.open_time = req['open_hour'] + req['open_minute']
        store.close_time = req['close_hour'] + req['close_minute']
        store.save()
...
def coupon_list(request, c_type, page_number=1):
    ...
    if c_type == 'registerwait':
        # 기존상품등록.
        total_count = cp.filter(activated_date_time__lte=today).filter(expired_date_time__lte=today).count()
        dic = get_page_list(request, page_number, total_count=total_count)
        documents = cp.filter(activated_date_time__lte=today).filter(expired_date_time__lte=today)[int(dic['start_offset']):int(dic['end_offset'])]
    elif c_type == 'publishwait':
        # 발행대기 . 시작일이 오늘보다 앞섰기때문에 발행시작이 안된 쿠폰. 그리고 만료일도 오늘보다 크다.
        documents = cp.filter(activated_date_time__gt=today).filter(expired_date_time__gt=today).filter(publish_flag=True)
        documents = cp.filter(expired_date_time__gt=today, publish_flag=True)
        arr_id = []
        tmp_documents = documents.filter(activated_date_time__lte=today)
        # tmp_documents 는 현재시간이 포함됨.
        for coupon in tmp_documents:
            if check_coupon_time(coupon):
                #만약에 오늘요일발행이아니라면 제외할 필요없음.
                if str(today_week) in str(coupon.weeks):
                    documents = documents.exclude(id=coupon.id)

        total_count = documents.count()
        dic = get_page_list(request, page_number, total_count=total_count)
        documents = documents[int(dic['start_offset']):int(dic['end_offset'])]
        ...
        # 발행중지된 쿠폰.
        total_count = cp.filter(publish_flag=False, expired_date_time__gt=today).count()
        dic = get_page_list(request, page_number, total_count=total_count)
        documents = cp.filter(publish_flag=False, expired_date_time__gt=today)[int(dic['start_offset']):int(dic['end_offset'])]
    else:
        if c_type != 'search':
...
def get_times(dictionary):
    ...
    for n in range(2014, today.year + 1):
