...
def member_register(request):
    ...
    if request.method == "GET":
        data = request.GET
        # data = {'member_id': "359670030454513", 'member_nickname': 'sky vega'}
    ...
    if 'isResponse' in data:
        ...
        #return HttpResponse(json.dumps(member_register_save(request, '1234')).encode('utf-8'))
...
def store_detail(request):
    if request.method == "POST":
        ...
        json_data = {'member_id': '1', 'store_id': '1', 'type': 'merchant'}
...
def coupon_list(request):
    ...
    if 'type' in json_data:
        if json_data['type'] == 'my':
            # 내 쿠폰 리스트
            arr = MyCoupon.objects.filter(member=Member.objects.get(id=int(json_data['id'])), useable=True).order_by('-regdate')
            coupon = []
            for item in arr:
                if item.coupon.activated_date_time <= today and item.coupon.expired_date_time >= today:
            ...
            if len(store_check(store)) > 0:
                coupon = []
                coupons = Coupon.objects.filter(visible_flag=True, publish_flag=True, activated_date_time__lte=today, expired_date_time__gt=today, store=store, weeks__contains=today.weekday()).order_by('-regdate')
                for cp in coupons:
            ...
            for store in store_list:
                coupons = Coupon.objects.filter(store=store, name__contains=json_data['query'], visible_flag=True, publish_flag=True, activated_date_time__lte=today, expired_date_time__gt=today, weeks__contains=today.weekday()).order_by('-regdate')
                for cp in coupons:
                    mycoupon_cnt = MyCoupon.objects.filter(coupon=cp).count()
                    if cp.count > mycoupon_cnt and check_coupon_time(cp):
                        coupon.append(cp)
            ...
            coupon = []
            for store in store_list:
                coupons = Coupon.objects.filter(store=store, visible_flag=True, publish_flag=True, activated_date_time__lte=today, expired_date_time__gt=today, weeks__contains=today.weekday()).order_by('-regdate')
                for cp in coupons:
                    mycoupon_cnt = MyCoupon.objects.filter(coupon=cp).count()
                    if cp.count > mycoupon_cnt and check_coupon_time(cp):
    ...
    for item in coupon:
        ...
        dic['act'] = str(item.activated_date_time).replace(" ", "-").replace(":", "-")
        dic['exp'] = str(item.expired_date_time).replace(" ", "-").replace(":", "-")
        dic['org'] = item.original_price
        dic['dis'] = item.discount_price
        dic['rate'] = item.discount_percentage
        dic['lat'] = item.store.latitude
        dic['lng'] = item.store.longitude
        ...
        if json_data['type'] == 'merchant' or json_data['type'] == 'merchant_search':
            ...
            dic['comment'] = status
            if status == u'발행대기':
                if item.activated_date_time <= today and today < item.expired_date_time:
                    # 발행대기 리스트에 있으나 이미 한번발행한 쿠폰.
                    dic['status'] = 22
                else:
                    dic['status'] = 2
            elif status == u'발행중':
...
def coupon_detail(request):
    if request.method == "POST":
        ...
        json_data = {'coupon_id': 88, 'member_id': 1, 'type': ''}
...
def coupon_add(request):
    if request.method == "POST":
        ...
        json_data = {'member_id': 1, 'coupon_id': 105}
    ...
    today = datetime.datetime.today()
    coupon = Coupon.objects.filter(visible_flag=True, publish_flag=True, activated_date_time__lte=today, expired_date_time__gt=today, id=int(json_data['coupon_id']))
    ...
    mycoupon = MyCoupon.objects.filter(coupon=coupon, member=member)
    mycoupon = mycoupon.filter(regdate__year=int(today.year), regdate__month=int(today.month), regdate__day=int(today.day), useable=True)
    if len(mycoupon) > 0:
        result['result'] = 'fail'
        result['msg'] = u'해당 사용자는 해당 쿠폰을 이미 등록하였습니다.'
        return HttpResponse(json.dumps(result).encode('utf-8'))

    mycoupon = MyCoupon()
    mycoupon.coupon = coupon
    mycoupon.member = member
    mycoupon.subscription_flag = subscription_flag
    mycoupon.save()
