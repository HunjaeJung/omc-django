...
def get_coupon_status(cp):
    ...
    if cp.activated_date_time <= today and cp.expired_date_time <= today:
        status = u"기존상품"
    if cp.expired_date_time > today and cp.publish_flag == True:
        status = u"발행대기"
        if cp.activated_date_time <= today and check_coupon_time(cp) and str(today_week) in str(cp.weeks):
            status = u"발행중"
    elif cp.activated_date_time <= today and cp.expired_date_time > today and cp.publish_flag == True:
        if check_coupon_time(cp) and str(today_week) in str(cp.weeks):
            status = u"발행중"
    elif cp.publish_flag == False and cp.expired_date_time > today:
        status = u"발행중지"
    ...
    if len(store) != 1:
        ...
        if store.activate_flag == True:
            ...
            if len(mybill) > 0:
                ...
                # 발행중인 등록된 쿠폰의 개수들의 합.
                coupons = Coupon.objects.filter(store=store, visible_flag=True, expired_date_time__gte=today, subscription_flag=False)
                total_cnt = Coupon.objects.filter(store=store, visible_flag=True, expired_date_time__gte=today, subscription_flag=False).aggregate(Sum('count'))['count__sum']
